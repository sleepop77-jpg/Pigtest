...
private fun DrawScope.drawScholarHalo(headCenter: Offset, w: Float, h: Float, phase: Float) {
    ...
    drawOval(
        color = Color(0xFFFFD700).copy(alpha = 0.22f * pulse),
        topLeft = Offset(haloCx - rx * 1.5f, haloCy - ry * 1.5f),
        size = Size(rx * 3f, ry * 3f)
    )
    drawOval(
        color = Color(0xFFFFD700),
        topLeft = Offset(haloCx - rx, haloCy - ry),
        size = Size(rx * 2f, ry * 2f),
        style = Stroke(width = 3.5f.dp.toPx(), cap = StrokeCap.Round)
    )
    drawOval(
        color = Color(0xFFFFF3C4).copy(alpha = pulse),
        topLeft = Offset(haloCx - rx, haloCy - ry),
        size = Size(rx * 2f, ry * 2f),
    ...
    for (i in 0 until 3) {
        ...
        drawCircle(
            color = Color(0xFFFFF9C4).copy(alpha = pulse),
            radius = 2f.dp.toPx(),
            center = Offset(haloCx + cos(ang) * rx * 1.45f, haloCy + sin(ang) * ry * 1.45f)
        )
    }
}
...
private fun DrawScope.drawNinjaHeadband(headCenter: Offset, w: Float, h: Float, phase: Float) {
    ...
    drawRoundRect(
        ...
        topLeft = Offset(headCenter.x - hrX * 0.98f, bandY),
        size = Size(hrX * 1.96f, bandH),
        cornerRadius = androidx.compose.ui.geometry.CornerRadius(bandH / 2f)
    )
    ...
    for (tail in 0 until 2) {
        ...
        drawPath(
            path,
            color = Color(0xFFD32F2F).copy(alpha = if (tail == 0) 0.95f else 0.7f),
...
private fun DrawScope.drawPartyMode(cx: Float, cy: Float, w: Float, h: Float, headCenter: Offset, phase: Float) {
    ...
    drawPath(
        ...
        brush = Brush.linearGradient(
            listOf(Color(0xFF9C27B0), Color(0xFFF5A623)),
            start = Offset(headCenter.x, hatBaseY - hatH),
            end = Offset(headCenter.x, hatBaseY)
        )
    )
    ...
    val colors = listOf(
        Color(0xFFFFD700), Color(0xFFD9534F), Color(0xFF20B2AA),
        Color(0xFF9C27B0), Color(0xFF00BCD4), Color(0xFFF5A623)
    )
    for (i in 0 until 12) {
        ...
        if (i % 3 == 0) {
            drawCircle(col, 2.2f.dp.toPx(), Offset(x, y))
        } else {
            drawRoundRect(
                col,
                topLeft = Offset(x - 2f.dp.toPx(), y - 1.2f.dp.toPx()),
                size = Size(4f.dp.toPx(), 2.4f.dp.toPx()),
...
fun AnimatedMascotPreview(skinId: String, size: Dp = 56.dp) {
    ...
    Canvas(Modifier.size(size)) {
        ...
        drawOval(
            ...
            topLeft = Offset(headCenter.x - w * 0.26f, headCenter.y - h * 0.24f),
